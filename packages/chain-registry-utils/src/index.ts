...
export const getIbcInfo = (ibc: IBCInfo[], fromChain: string, toChain: string): IBCInfo => {
  return findInfo(ibc, fromChain, toChain)
    || findInfo(ibc, toChain, fromChain);
};
...
export const getIbcDenom = (
  ...
  const ibcInfo = getIbcInfo(ibc, fromChain, toChain);
  if (ibcInfo) {
    ...
    let channelInfo;
    if (ibcInfo['chain-1']['chain-name'] === fromChain) {
      channelInfo = channel['chain-1'];
    } else {
      channelInfo = channel['chain-2'];
    }

    const assetInfo = assets.find(({ chain_name }) => chain_name === toChain);
    if (!assetInfo) {
      return;
    }
